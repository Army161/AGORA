...
2. Add this plugin to Claude Code (plugin marketplace pointing at this folder, or copy
   into your plugins dir). On session start, `.mcp.json` launches the server with workspace
   `./.agora` in your current project.
3. Verify: run `/agora-board`. First call bootstraps `./.agora/`.
